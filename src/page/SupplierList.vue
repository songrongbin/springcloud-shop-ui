<template>
  <div class="fillcontain">
    ...
    <div class="filter-container">
      <el-input v-model="query.supplierCode" clearable class="filter-item" style="width: 200px;" placeholder="请输入供应商编码"/>
      <el-input v-model="query.supplierName" clearable class="filter-item" style="width: 200px;" placeholder="请输入供应商名称"/>
      ...
      <el-table
        ...
        <el-table-column align="center" label="操作" width="400" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button type="primary" @click="viewRecordDetail(scope.row)">供应商详情</el-button>
            <el-button type="primary" @click="deleteRecord(scope.row)">删除供应商</el-button>
            <el-button type="primary" @click="viewEditRecord(scope.row)">编辑供应商信息</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :visible.sync="editDialogVisible" title="部门编辑" width="700">
      <el-form :data="editInfo" label-position="right">
        <el-form-item label="用户组编码" label-width="100px">
          <el-input v-model="editInfo.supplierCode"></el-input>
        </el-form-item>
        <el-form-item label="用户组名称" label-width="100px">
          <el-input v-model="editInfo.supplierName"></el-input>
        </el-form-item>
        <el-form-item label="供应商图片" label-width="100px">
...
import {getSupplierList, getSupplierDetail, deleteSupplier, addSupplier, editSupplier} from '@/api/getData'
...
export default {
  ...
  methods: {
    ...
    async viewRecordDetail (row) {
      const detailResult = await getSupplierDetail({id: row.id})
    ...
    deleteRecord (row) {
      this.$confirm('删除操作不能恢复，您确定要删除吗?', '提示', {
        ...
        const result = deleteSupplier({id: row.id})
        if (result.code === 0) {
          this.$message({
            type: 'success',
            message: '删除成功!'
    ...
    async doEdit (row) {
      if (row.id == null || row.id === 0) {
        const result = await addSupplier(this.editInfo)
        if (result.code === 0) {
        ...
        const result = await editSupplier(this.editInfo)
        if (result.code === 0) {
          this.editDialogVisible = false
          this.$message({
            type: 'success',
    ...
    async getSuppliers () {
      const listResult = await getSupplierList({pageNum: this.offset, pageSize: this.limit, supplierCode: this.query.supplierCode, supplierName: this.query.supplierName})
